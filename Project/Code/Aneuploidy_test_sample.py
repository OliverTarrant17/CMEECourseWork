...
for sample in Included_Samples:
    ...
    Overall_Prob=np.zeros((NContigs+1,len(ploidy)),float) # (NSAMS+1)xploidies array for probabilities of each ploidy for each sample and overall ploidy probabilities
        ...
        # After going through all samples record information for chunks and 
                ...
                if base_number%win==0: #calculate most likely ploidy for each 50 biallelic bases chunks of supercontig
                    ...
                    if max(Overall_Prob_HWE[contig])<0:
                        ...
                        print(Overall_Prob_HWE[contig])                          
                    Overall_Prob[0]+=Overall_Prob_HWE[0] # Keep a track of the overall probability of each ploidy at each sample
