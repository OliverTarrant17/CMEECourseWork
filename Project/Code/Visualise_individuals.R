...
for(i in 1:NSAMS){  
  ...
  plot <- plot + geom_point(data=data_to_plot,aes(x=c(1:length(data_to_plot$value)),y=data_to_plot$value/Meandepth,colour=factor(expected_ploidy)),alpha=1/2)
  ...
  plot <- plot + scale_y_continuous('Normalised Depth', limits=c(0,normalised_haploid*max(data_to_plot$ploidy_num)+0.5), sec.axis = sec_axis(~./normalised_haploid,name = 'Inferred Ploidy',breaks = c(0:max(data_to_plot$expected_ploidy)+1))) # add 2nd y axis
